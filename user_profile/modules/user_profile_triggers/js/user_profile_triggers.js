(function ($) {
  ...
  Drupal.SallyHansen_User_Profile_triggers = Drupal.SallyHansen_User_Profile_triggers || {};


  Drupal.SallyHansen_User_Profile_triggers.action = function(action,variable) {
    switch (action) {
      case 'popup':
        var url = variable;
        switch (url) {
          case '/sh_newsletter/form/nojs':
            Drupal.SallyHansen_User_Profile_triggers.auto_open(url);
            // Drupal.settings.SallyHansen_User_Profile_triggers.user_profile_return_visitor = 1;
  ...
  Drupal.SallyHansen_User_Profile_triggers.auto_open = function(url) {
      Drupal.Dialog.show();
  ...
  Drupal.behaviors.user_profile_triggers = function(context) {
    ...
    if (context =='[object HTMLDocument]' && Drupal.settings.user_profile_triggers.newsletter_signup_flag == 1)  {
      Drupal.SallyHansen_User_Profile_triggers.action('popup','/sh_newsletter/form/nojs');
    }

    if (context =='[object HTMLDocument]' && Drupal.settings.user_profile_triggers.user_profile_update_flag == 1)  {
      Drupal.SallyHansen_User_Profile_triggers.action('popup','/user_profile_reminder/nojs');
    }
